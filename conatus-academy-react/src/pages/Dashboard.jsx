...
export function Dashboard() {
  ...
  const enrichedMatriculas = useMemo(() =>
    matriculas.map(m => {
      if ((m.curso_id === MOP_ID || m.curso_id === '6') && m.tipo === 'estatico') {
  ...
  const hasMopEnrollment = useMemo(() =>
    enrichedMatriculas.some(m => m.curso_id === MOP_ID || m.curso_id === '6'),
    [enrichedMatriculas]);

  // MOP só é "concluído" quando 100% aulas + quiz aprovado
  const concluidos = enrichedMatriculas.filter(m => {
    if (m.curso_id === MOP_ID || m.curso_id === '6') return mopCertOk;
    ...
    <div className="dashboard-body">
      <div className="dashboard-container">
          ...
          <section className="mop-panel">
            ...
            <div className="mop-status-grid">
              ...
              <div className={`mop-status-card ${mopQuiz.passed ? 'done' : mopQuiz.attempts > 0 ? 'fail' : 'pending'}`}>
                ...
                {mopQuizOk.ok && (
                  <Link to="/cursos/mop-interno/avaliacao" className="mop-status-action">
        ...
        <section className="courses-section" style={{ marginTop: '30px' }}>
          ...
          {(() => {
            const otherCourses = enrichedMatriculas.filter(m => m.curso_id !== MOP_ID && m.curso_id !== '6');
            if (otherCourses.length === 0) return (
              <div style={{ background: 'white', padding: '40px', textAlign: 'center', borderRadius: '8px', border: '1px solid var(--border)' }}>
...
// Emissão de certificado — chama API se disponível, senão simula
async function handleEmitCert() {
  try {
    const { api } = await import('../services/api');
    await api.emitirCertificado(MOP_ID, 80);
    alert('Certificado emitido com sucesso! Verifique seu e-mail.');
  } catch {
    alert('Certificado disponível! Entre em contato com a administração.');
  }
}
